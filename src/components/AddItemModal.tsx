  ...
  return (
    <Modal title={item ? "Modifier l'article" : "Ajouter un article"} maxWidth="xl" onClose={onClose}>
      <form onSubmit={handleSubmit} className="grid gap-4 p-5">
        <Field label="Nom">
          <TextInput name="name" defaultValue={item?.name ?? ""} />
        </Field>
        <Field label="Note">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Catégorie">
            <SelectInput name="category_id" defaultValue={item?.category_id ?? categories[0]?.id}>
          ...
          <Field label="Assigné à">
            <TextInput name="assigned_to" defaultValue={item?.assigned_to ?? ""} />
          </Field>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Prix estimé">
            <TextInput name="price_estimate" type="number" min="0" step="0.01" defaultValue={item?.price_estimate ?? ""} />
          </Field>
        </div>
