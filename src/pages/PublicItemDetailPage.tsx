...
function DetailHeroImage({ item, alt }: { item: Item; alt: string }) {
  ...
  return (
    <div className="img-placeholder relative flex aspect-[4/3] max-h-[min(52vh,520px)] w-full items-center justify-center overflow-hidden rounded-[24px] shadow-inner">
      ...
      <span className="relative font-mono text-xs tracking-[0.08em] text-[oklch(68%_0.16_295)] opacity-80">photo produit</span>
    </div>
  );
}
  ...
  return (
    ...
    >
      <div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-[14px] bg-[oklch(96%_0.03_295)]">
        {link.image ? (
          <img src={link.image} alt="" className="h-full w-full object-cover transition duration-300 group-hover:scale-105" />
        ) : (
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-center gap-1">
        ...
        {link.price ? (
          <p className="mt-0.5 text-sm font-bold text-[oklch(52%_0.16_82)]">{link.price}</p>
        ) : null}
    ...
    return (
      <WishlistPageShell>
        <div className="mx-auto max-w-lg px-6 py-20">
          <Notice tone="error" className="p-5">{wishlist.error.message}</Notice>
          <Link to="/" className="mt-4 inline-block text-sm font-semibold text-[oklch(55%_0.16_295)] underline-offset-2 hover:underline">
            Retour à la liste
          </Link>
        </div>
      </WishlistPageShell>
    ...
    return (
      <WishlistPageShell>
        <div className="mx-auto max-w-lg px-6 py-20 text-center">
          ...
          <h1 className="font-['Cormorant_Garamond'] text-2xl text-[oklch(45%_0.12_295)]">Article introuvable</h1>
          <p className="mt-2 text-sm text-[oklch(50%_0.10_295)]">Ce cadeau n’existe pas ou n’est plus sur la liste.</p>
          <Link
            to="/"
      ...
      <div className="relative z-[1] mx-auto w-full max-w-[800px] px-4 pb-28 sm:px-6 sm:pb-20">
        <header className="animate-detail-in">
          <nav className="flex flex-wrap items-center gap-2 py-4 text-sm" aria-label="Fil d’Ariane">
            ...
            <span className="min-w-0 max-w-[40vw] truncate font-medium text-[oklch(50%_0.10_295)]" title={categoryName}>
              {categoryName}
            </span>
            <span className="text-[oklch(70%_0.08_295)]" aria-hidden="true">
              /
        ...
        <article>
          ...
          <div style={{ animationDelay: "120ms" }} className="mt-8 animate-detail-in">
            <div className="mb-3 flex flex-wrap items-start gap-3">
              ...
              {reserved ? (
                <span className="shrink-0 rounded-full bg-[oklch(88%_0.06_160)] px-3.5 py-1.5 text-xs font-bold tracking-[0.04em] text-[oklch(38%_0.12_160)]">
                  Réservé
                </span>
              ) : null}
            ...
            {item.note ? (
              <p className="text-[16px] leading-relaxed text-[oklch(45%_0.10_295)] [text-wrap:pretty]">{item.note}</p>
            ) : null}

            {item.price_estimate !== null ? (
              <p className="mt-5 font-['Cormorant_Garamond'] text-3xl font-medium text-[oklch(55%_0.14_82)]">~{item.price_estimate} €</p>
            ) : null}
          </div>
          ...
          {item.links.length > 0 ? (
            ...
            >
              ...
              <p className="mb-4 text-sm text-[oklch(50%_0.10_295)]">Liens vérifiés par les parents — ouvrez-les pour voir le détail, la taille, les couleurs, etc.</p>
            ...
            <p style={{ animationDelay: "200ms" }} className="mt-8 text-sm italic text-[oklch(55%_0.10_295)] animate-detail-in">
          ...
          {reserved ? (
            <p style={{ animationDelay: "240ms" }} className="mt-8 text-center text-sm text-[oklch(50%_0.10_160)] animate-detail-in">
